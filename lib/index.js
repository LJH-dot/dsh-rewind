...
// surface 占位节点：空内容 user/message 是 surface-eligible 且 token meter 不校验
// user/message 是否在 step 内（assistant/message 会被 token meter 强制要求匹配
// step/start，否则 /compact 直接 internal error）。占位文本为空，模型上下文不会
// 看到任何 rewind/unrewind 字样、保险柜路径或回退标记。
...
function placeholderEventData(id, model) {
  return {
    ...
    source: { kind: "plugin", plugin: PLACEHOLDER_PROVIDER, marker: model }
  };
}
...
function isDshPlaceholder(event, model) {
  ...
  return event?.type === "user/message"
    && source?.kind === "plugin"
    && source.plugin === PLACEHOLDER_PROVIDER
    && source.marker === model;
}
...
function apply(ctx) {
  ...
  ctx.on(
    "session/event",
    (session, event) => {
      // 回合以 provider 的“孤儿 tool 消息”错误结束时，自动清掉孤儿 tool/result。
      // 用户不需要知道 repair 存在，也不用手动敲命令。
      const reasonMessage = event.data?.reason?.error?.message ?? "";
      if (event.type === "turn/end"
        && event.data?.reason?.kind === "error"
        && reasonMessage.includes("Messages with role 'tool'")) {
        setTimeout(() => {
          repairOrphanToolResults(session).catch((error) => {
            if (ctx.logger?.warn) ctx.logger.warn(`dsh-rewind: auto repair failed: ${String(error)}`);
          });
        }, 0);
        return;
      }
...
async function rewindTo(session, input) {
  ...
  // 自动修一下历史遗留的孤儿 tool/result，用户无感知。
  await repairOrphanToolResults(session);
...
function findLatestRewindMarker(session) {
  ...
  for (let i = nodes.length - 1; i >= 0; i -= 1) {
    ...
    // 新格式：rewind 切面是空 user/message（source.marker=rewind-placeholder）。
    // 旧格式兼容：空 assistant/message（provider/model 标记）或 ↩ /rewind 文本。
    if (isDshPlaceholder(event, REWIND_PLACEHOLDER_MODEL)
      || (
        event?.type === "assistant/message"
        && event.data.message?.source?.provider === PLACEHOLDER_PROVIDER
        && event.data.message.source.model === REWIND_PLACEHOLDER_MODEL
      )
      || (
        event !== undefined
        && event.type === "user/message"
